...
# BACKEND FUNCTIONS
from funcs_alarm.triggeralarmImpl import TriggeralarmImpl
from funcs_alarm.alarmImpl import AlarmImpl

# CALLABLE OBJECTS
from objects_alarm.alarm import Alarm
...
#/restconf/operations/triggerAlarm/
class TriggeralarmMethodView(MethodView):

    def post(self, ):
        print "Create operation of resource: triggerAlarm"
        response = TriggeralarmImpl.post( )
        return Successful('Successful operation','{"description":"Create operation of resource: triggerAlarm"}')


#/restconf/config/alarm/
class AlarmMethodView(MethodView):

    def get(self, ):
        print "Retrieve operation of resource: alarm"
        try:
            response = AlarmImpl.get()
        except KeyError as inst:
...
getattr(sys.modules[__name__], __name__).add_url_rule("/restconf/operations/triggerAlarm/", view_func = globals()["TriggeralarmMethodView"].as_view('"Triggeralarm"'+'"_api"'), methods=['POST'])
getattr(sys.modules[__name__], __name__).add_url_rule("/restconf/config/alarm/", view_func = globals()["AlarmMethodView"].as_view('"Alarm"'+'"_api"'), methods=['GET'])
